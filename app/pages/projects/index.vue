...
<template>
    <UContainer>
        <h2 class="text-xl sm:text-3xl font-bold text-left"> My Projects. </h2>

        <div class="flex flex-wrap gap-2 sm:justify-center my-4">
            <UButton v-for="type in types" :key="type" :label="type" :color="selectedType === type ? 'primary' : 'gray'"
                @click="selectedType = type" variant="soft" size="sm" />
        </div>
